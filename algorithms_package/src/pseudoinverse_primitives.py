...
def processTensorsTakeLeft(psi_tensors: typing.List[np.array], phi_tensors: typing.List[np.array]):
    ...
    for i in range(0, len(modes) - 1):
        ...
        new_shape = (phi_tensors[i - 1].shape[0], psi_tensors[i].shape[1], psi_tensors[i].shape[0])
        p = lstsq(phi_tensors[i - 1].T, R, eps=1e-10).reshape(new_shape, order='F')
...
def processTensorsTakeRight(psi_tensors: typing.List[np.array], phi_tensors: typing.List[np.array]):
    ...
    for i in range(1, len(modes)):
        ...
        answer.append(lstsq(phi_tensors[i - 1], R, eps=1e-10).reshape(new_shape, order='F'))
    return answer
